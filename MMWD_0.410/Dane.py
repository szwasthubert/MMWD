    ...
    def generate_neighborhood(self, taboo_list: dict, timeout :int  = 5):
        ...
        def generate_neighborhood_economically(self: Solution, taboo_list):
            ...
            try:
                ...
                while best_generator.ID in (x[0] for x in taboo_list['economically']) and i!=len(self.generators)-1:
                    i+=1
            ...
            worst_generator = next(iter(reversed_generators))
            # Aspiration criterion
            try:
                while worst_generator.ID in (x[0] for x in taboo_list['economically']):
                    worst_generator = next(iter(reversed_generators))
            except StopIteration:  # Everything in taboo list
                pass
            ...
            for interval in intervals:
                ...
                for sub_interval in interval:
                    new_work_matrix[best_generator.ID,sub_interval[0]:sub_interval[0]+sub_interval[1]] = 1
                    new_work_matrix[worst_generator.ID, sub_interval[0]:sub_interval[0] + sub_interval[1]] = 0
        ...
        def generate_neighborhood_renewably(self, taboo_list,timeout):
            ...
            best_generator = next(iter(self.generators))
            # Aspiration criterion
            try:
...
def generate_random_intervals_renewable(minimal_working_time, time_size, interval_quantity, power_production):
    ...
    for start, length in zip(starts, lengths):
        ...
        interval_sum.append(list(interval_set))
